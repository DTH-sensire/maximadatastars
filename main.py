...
def GenerateSummary():
    ...
    eerste_samenvatting = ({"ondersteuningsbehoeften": itemgetter("ondersteuningsbehoeften"), "dagrapportages": itemgetter("dagrapportages")} | PromptTemplate.from_template(ini_temp) | model_parser)
    feedback_samenvatting = ({"dagrapportages": itemgetter("dagrapportages"), "base_output": itemgetter("base_output")} | PromptTemplate.from_template(feedback_temp) | model_parser)
    opti_samenvatting = ({"feedback": itemgetter("feedback"), "base_output": itemgetter("base_output")} | PromptTemplate.from_template(opti_temp) | model_parser)

    chain = ({'base_output': eerste_samenvatting, "dagrapportages": itemgetter("dagrapportages")} | RunnablePassthrough.assign(feedback=feedback_samenvatting) | opti_samenvatting)

    final_output = chain.invoke({"ondersteuningsbehoeften": docs_ob, "dagrapportages": docs_dr})
    return(final_output)
